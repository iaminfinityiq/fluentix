...
func not_eof(tokens *[]lexer.Token, file_extension string) bool {
	return !is_eof(tokens, file_extension)
}
...
func parse_indented_block(tokens *[]lexer.Token, file_extension string) runtime.RuntimeResult {
	...
	if file_extension == "flu" {
		...
		var block Block = Block{Body: []Statement{}}
		for at(tokens, file_extension).TokenType != lexer.End && not_eof(tokens, file_extension) {
			rt = parse_statement(tokens, file_extension)
			if rt.Error != nil {
				return runtime.Failure(*rt.Error)
			}
		...
		if is_eof(tokens, file_extension) {
			return runtime.Failure(runtime.SyntaxError{Reason: fmt.Sprintf("Expected %d, got '%s'", lexer.End, at(tokens, file_extension).Value)})
		}
...
func parse_statement(tokens *[]lexer.Token, file_extension string) runtime.RuntimeResult {
	...
	if not_semicolon_or_eof(tokens, file_extension) {
